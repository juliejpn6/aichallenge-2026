...
- x, y座標のidx252(前後数点)を、より広い局所窓で補間・平滑化する
  最小限の修正を検討する。コリドー計算ロジック・制御コードには一切
  触れない、レースラインCSVのみの修正。
- レースラインCSV(競技提出物の中核データ)の修正はQ/Rパラメータとは
  性質が異なるため、①具体的な補間計算→②修正後の全区間kappaプロファイル
  再チェック→③回帰スイート→④専用のdev3検証、という手順を踏む。
- 修正後はQ[e_y]等の最適値もwp252抜きで再評価が必要になる可能性がある
  (wp252の異常自体がこれまでの一部パラメータ選択に影響していた可能性)。
