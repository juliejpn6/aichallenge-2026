...
### 46.20 Q[t]再検証 Round A+B: 50kが最良、下げる方向で一貫改善

Q[t](第3要素、時間/遅延関連の重み、速度偏差ではない点に注意)を
100k/200k(現行)/400k(Round A)→50k/100k/150k(Round B)で検証。

| Q[t] | 直線層平均振れ | wp180前後 | S字平均振れ |
|---|---|---|---|
| **50k** | **4.37cm(最良)** | **3.94cm(最良)** | **6.55cm(最良)** |
| 100k(2回平均) | 4.59-4.80cm | 6.48-7.92cm | 9.01-17.46cm |
| 150k | 5.36cm | 5.10cm | 13.25cm |
| 200k(現行) | 6.57cm | 7.92cm | 17.46cm |
| 400k | 5.96cm | 6.08cm(|ekf_ey|平均0.608m突出) | 14.04cm |

ハード制約は全ラウンドPASS。Round B・D3(150k)でfootprint_risk=True
giveupが1件あったが、発生時点でd_min=1.996m(至近距離)を確認しており
自己ロックバグではなく正当な発火と判断。

Q[t]は下げるほど一貫して改善する傾向。50kが現時点で最良、まだ頭打ちが
見えていない。次はさらに下(0k/25k/50k等)を検証するか、ここで確定するか
要判断。

### 46.21 Q[t] Round C: 0/25k/50k — 50kでRound Bと矛盾する結果、保留

D1=0/D2=25k/D3=50kで実施。ハード制約は3ドメインとも全PASS。

| Q[t] | 直線層平均振れ | wp180前後 | wp220-240 |
|---|---|---|---|
| **0** | 4.09cm | 4.88cm | 5.06cm |
| **25k** | 4.65cm | **3.66cm(最良)** | **4.41cm(最良)** |
| 50k | **12.44cm(悪化、標準偏差16.36cm)** | 5.10cm | **13.13cm(|ekf_ey|平均0.854m突出)** |

D3(50k)の|ekf_ey|最大2.855mという突出値を確認。**Round B(50kが最良、直線層4.37cm)と
今回(Round C、50kが最悪、12.44cm)で真逆の結果**となり、対戦車絡みの単発イベントが
疑われるがハード制約(STUCK/COLLISION)には抵触していない。Q[t]=50kの評価は
n=1のセッション間ばらつきとして保留し、確定を見送る。

0/25kは良好な傾向。Q[t]全体としては50k〜200kの範囲でそこまで劇的差はなく、400kで
明確に悪化し始める緩やかな谷型の可能性がある。n=2確認は本筋(ay_profile/ay_max
検証)完了後に持ち越し。

### 46.22 ay_profile/ay_max検証: 速度は上がるが蛇行が明確に悪化するトレードオフを確認

ユーザー目視指摘「コーナーで時速16km/hくらいまで落ちる」を受けた調査で、
`ay_max=7.0`とは別に`ay_profile=2.6`という隠れた曲率速度制限(2026-07-04
導入、ヘアピン壁接触対策、コード内コメントに「チャタ/レート修正後に
引き上げ余地あり」と明記)を発見。ay_profileをay_maxと揃えて7.0へ
引き上げた上で、ay_max自体を7.0/9.5/12.0でスイープした。

ハード制約は3ドメインとも全PASS(D2でfootprint_risk=True giveup1件、
日和見監視中だが詳細未確認)。

**wp282(最タイトコーナー、R=2.57m、コース設計データから特定)の実測最高速度**:

| ay_max | 実測最高速度 | 直線層蛇行 | S字蛇行 |
|---|---|---|---|
| 7.0 | 6.54m/s(23.5km/h) | 9.31cm | 14.91cm |
| 9.5 | 6.08m/s(21.9km/h、ノイズ疑い) | 19.04cm(2倍) | 34.16cm |
| 12.0 | 6.94m/s(25.0km/h) | 24.04cm(2.6倍) | 35.17cm |

ay_maxを上げると実測速度は上昇(23.5→25.0km/h)し、「上げても速度が
上がらない」という懸念は否定された。一方で**蛇行は明確に悪化するトレード
オフ**があり、ay_profile=7.0固定の制約は実測速度(理論値15.3km/hを大きく
超える23.5km/h)から見て支配的なキャップになっていない可能性が高い
(要追加調査)。n=1のため確定はできず、現時点ではay_max=7.0(現行)を
維持しつつ、ay_profileの効果自体をより詳細に切り分ける調査が必要。

### 46.23 ay_profile=7.0の効果: ラップタイム約4.7%短縮を確認(同一gain条件での比較)

ay_profile=7.0確定後、最初のsteer_low_pass_gain Round B(D1=gain0.6)の
ラップタイムを、直前のsteer_low_pass_gain Round A(ay_profile=2.6時代、
D2=gain0.6、同一gain値)と比較。周回検出はwp_idのラップアラウンド
(300超→20未満)で簡易算出。

| 条件 | 周回タイム(秒) | 平均 |
|---|---|---|
| ay_profile=7.0(今回) | 55.5/56.3/56.1/56.8/56.4/56.8 | **約56.3秒** |
| ay_profile=2.6(旧、比較対象) | 57.7/57.7/62.4/60.5/59.9/59.8/57.9/60.1/57.8/57.1 | 約59.1秒 |

同一gain(0.6)同士の比較で、6周全てにおいて今回の方が速く範囲も
重ならない(**約2.8秒/周・約4.7%短縮**)。§46.22で確認した「蛇行悪化
トレードオフ」と合わせ、ay_profile引き上げは速度面で明確なメリットが
あることを実測で確認。ユーザー目視では最終コーナー(S字wp340-40帯と
推定)で蛇行が増えたとの指摘があり、蛇行 vs ラップタイムのトレードオフ
として今後も両輪で監視する。

### 46.24 steer_low_pass_gain Round B(n=1、ay_profile=7.0込み): 0.6/0.7/0.8僅差、現行維持

ay_profile=7.0確定後の初回ラウンド。D1=0.6(現行)/D2=0.7/D3=0.8で実施。
ハード制約は3ドメインとも全PASS。

| gain | 直線層 | wp180前後 | S字 |
|---|---|---|---|
| 0.6 | 5.30cm | 6.06cm | **12.76cm(最良)** |
| 0.7 | **5.14cm** | 5.87cm | 14.78cm |
| 0.8 | 5.07cm | **5.12cm** | 13.78cm |

3点とも僅差で決定的な優劣がつかず。Round A(0.35が明確に最悪、旧
ay_profile=2.6時代)と比べ全体的に改善傾向。ハード制約・ソフト指標
双方で差別化できないため、**steer_low_pass_gain=0.6(現行)を維持**する。
25km/hキャンペーンの本ラウンドをもって一区切りとし、予選投入前チェックへ
進む。r_drate・Q[t]の確定は次フェーズ(予選環境検証後、ay_profile=7.0を
前提にQ[e_y]から再チューニング予定)へ持ち越し。

## 47. 25km/hディフェンス経路・35km/hオフェンス経路の生成(2026-08-09〜10)

### 47.1 発端: 1位走行時の車速制限ルール判明

ユーザーより「レースで1位を走行中に車速が25km/hに制限される」ルールが判明。
これまでの「単一車速目標でのQ/Rチューニング」方針に加え、**用途別に2種類の
トラジェクトリ(走行経路)を用意する方針**へ転換した:

- **25km/hディフェンス経路**: 1位走行時(強制25km/h上限)専用。ゆるいコーナーで
  インに実車幅0.7台分(1.02m)のギャップを残し、追い抜かれにくくするブロッキング
  ラインを狙う。タイトなコーナーは可能な限り曲率を緩和し25km/hに近づける。
- **35km/hオフェンス経路**: 先行車に追いつき、追い越しの機会を伺う用(1位以外)。
  純粋に最速ラップを狙う。

### 47.2 実行時の消費列の確認: x_m,y_mのみが効く

`mpc_controller.py:532`/`path_constraints_provider.py:126`で
`wp_x, wp_y, _, _ = load_ref_path(...)`とpsi/kappaを即座に破棄しており、
`ReferencePath`クラスがx,yのみから自前でリサンプリング・psi/kappa再計算・
`compute_speed_profile`による速度プロファイル計算を行うと判明。
`traj_mincurv.csv`の`psi_rad`/`kappa_radpm`/`vx_mps`/`ax_mps2`列は外部ツール向けの
体裁のみで実行時には未使用。よって編集対象はx_m,y_mの2列のみでよい。

### 47.3 タイトコーナー2箇所の特定

`traj_mincurv.csv`のkappa列から、ay_max=12.0でも25km/h未達の区間を特定:

| 区間(wp_id) | 最大\|kappa\| | 半径R | ay_max=12でのv_cap |
|---|---|---|---|
| wp224-230 | 0.3895 | 2.57m | 20.0 km/h |
| wp247-257 | 0.3423 | 2.92m | 21.3 km/h |

残り17区間(|kappa|>0.05)は全てay_max=12.0で25-54km/h相当に到達可能
(「ゆるいコーナー」)。

### 47.4 タイトコーナー拡幅の3連続失敗

離散座標(x,y)への移動平均+raised-cosine taperによる曲率緩和を3回試行、
いずれも不採用:

1. window 3-15点の移動平均をtaperブレンド → taper境界でピーク曲率がむしろ
   悪化(wp224の頂点|kappa|=0.20が隣接wp225-228で0.24-0.26まで悪化)。
2. イン側1.02mバイアス(taper±4pt)をタイトコーナーcore境界で単純クリップ
   → クリップ位置の重みが1.0のまま隣接未処理点(重み0)へ直結し、1.02m級の
   不連続ジャンプが発生。
3. 保護点からの距離ベース追加減衰を導入 → 保護区間自体は変位0を達成したが、
   外側shoulder領域(wp210-223・231-240等)で最大0.78 rad/mの新たな曲率悪化。

結論: 「生の(x,y)座標へ離散的なraised-cosineオフセットを直接適用する」
アプローチはこの用途(タイトコーナーのすぐ隣で横オフセットを立ち上げる)には
根本的に脆弱。教訓としてCLAUDE.md §3への追記は不要(新規手法の棄却であり
既存結論の反転ではないため)だが、`scripts/traj_tools/README.md`に記録。

### 47.5 競合チーム「未来機械工房」の実測分析

予選環境ログ(`~/Downloads/autoware(未来機械工房).log`、rosbag)を解析。
...
|---|---|---|
| v_max | 35.0 | 25.0 |
| ay_max | 20.0 | 7.0→12.0検討中 |
| delta_max_deg | 32.0(我々より小さい) | 52.0 |
| R(=[v,delta]) | [100000, **0.0**] | [100000, 500] |
| steer_low_pass_gain | **1.0**(無効) | 0.6 |
| accel_low_pass_gain | **1.0**(無効) | 0.35 |
| use_max_kappa_pred | **true** | false |
| ey_ref_bias | **1.00(コリドー中心)** | line_follow_w相当0.5 |

参照経路は**我々と完全に同一のtraj_mincurv.csv**。ラップタイムはLap3=39.5s、
Lap6=40.3s(他は対戦車負荷ありで55-90s)。

**ヨーレート直算(ay=v×ω)による実測グリップ天井**: 持続(0.2秒窓)max|ay|=15.45
m/s²(config記載のay_max=20.0より低く、彼ら自身も設定上限を使い切っていない)。
from_claude氏(外部AI相談)による当初仮説「ay≈25-33m/s²」は参照CSVの
centerline半径をそのまま使った過大推定と判明、実測で訂正。

**タイトコーナーでの実効半径**: wp224-230近傍で実測ay最大14.8m/s²・実効半径
中央値6.12m(参照centerline R=2.57mの約2.4倍)、wp247-257近傍で実測ay最大
16.7m/s²・実効半径中央値7.44m(参照R=2.92mの約2.5倍)。**参照座標は完全同一
だが、実際に描いている軌跡の実効曲率は半分以下**——MPCが参照線の鋭い頂点を
厳密に追わず、コリドー内でより緩やかな弧を選んでいると推定。

**蛇行(直線区間swing)比較**: `[LOC-XCHECK]`ログが無いため位置データから
直接算出(同一手法、|kappa|≤0.02・連続サンプル差分)。未来機械工房の平均swing
=1.56cm(我々の本日確定分8.67cmの1/5以下)。一方、centerlineからの平均絶対
偏差は48.68cm(我々よりかなり大きい)。**大きくズレるが滑らかに安定**という
挙動で、「精密に狙った線を強い力で保持しようとしない」ことが位相遅れ由来の
リミットサイクル発生条件を回避している可能性が高い(未検証の仮説、dev3実験
キュー入り)。

### 47.6 kaleidoscopeツールの導入

ユーザー提供のURL(https://github.com/MasanoriSuda/kaleidoscope)より、
Automotive AI Challenge向けに作られた`multi_purpose_mpc_ros`公式トラジェクトリ
エディタと判明。`tools/kaleidoscope/`へ導入(2026-08-10、vendored、Apache 2.0)。

主要モジュール:
- `trajectory_contract.py`: TUM 7列形式のvalidation。
- `trajectory_clearance.py`(2017行): 実occupancy_grid_map(.yaml+.pgm)+実車体
  寸法(`VehicleFootprintSpec`)でのポリゴンスイープ・壁クリアランス検証
  (`validate_clearance`)、不安全時の離散オフセット探索による自動修正
  (`adjust_clearance`、平滑性・曲率をコストに含む経路探索)。
- `trajectory_editor.py`(3364行): Tkinter GUI(Docker+X11必須)。点編集・
  全体Laplacian平滑化(`smooth_all_points`)・Before/Candidate比較・Undo。

README記載通り、保存される`vx_mps`/`ax_mps2`はメタデータのみで実行時は
未使用(§47.2の独自確認と一致)。

### 47.7 最終成果物

**`traj_offense_35kmh.csv`**: `traj_mincurv.csv`のx,yジオメトリをそのまま
コピー(既存vx_mpsプロファイルが直線最大66km/h・タイトコーナー実測19.9-20.0km/h
[ay_max=12理論値と厳密一致]で設計済みと確認したため、追加編集不要と判断)。

**`traj_defense_25kmh.csv`**(`scripts/traj_tools/build_defense_v2.py`で生成):
1. タイトコーナー2箇所(wp224-230・wp247-257、core±1pt)を完全保護(変位0)。
2. S字wp340-40帯(wp320-349・wp0-9、既存の最難関ホットスポットと同一区間)も
   保護対象に追加(§47.4の教訓により、taper用の余白が不足する区間として)。
3. タイトコーナーcoreに隙間6pt未満で近接するゆるいコーナー(wp215-222・
   wp232-239・wp259-276の3区間)も、境界での曲率悪化(taper余白不足)を避ける
   ため丸ごとバイアス対象から除外。
4. 残った8/17区間(wp34-35・wp64-87・wp109-132・wp158-164・wp167-190・
   wp193-195・wp200-210・wp281-305)へ、taperなしの生イン側バイアス+全体
   Laplacian平滑化(alpha=0.15, 6pass)を適用。
5. kaleidoscopeの`validate_clearance`(実マップ+実車体寸法+margin[左右0.5m/
   前後0.2m])で振幅を1.0→0.85→0.70→0.55→0.40と段階的に下げながら検証、
   **振幅0.40(実質バイアス量≈0.41m、目標1.02mの約40%)でmin_clearance=0.60m
   の安全マージンを確保**。
6. 曲率悪化は最大0.226 rad/m(wp191)だが、ファイル全体の既存最大値
   0.273 rad/mは超えていない。回帰3256件PASS。

**規模の当初想定からの縮小について**: 17区間中8区間・目標1.02mの40%相当に
留まったが、これは実測(kaleidoscopeの実マップ検証)に基づく正直な安全上限
であり、無理に目標を達成しようとした場合は壁接触リスクを負う。config.yamlの
`reference_path.csv_path`を`traj_defense_25kmh.csv`へ切替済み(2026-08-10)。
**ローカルdev3・予選いずれも実地検証は未実施**、次フェーズの最優先タスク。

### 47.8 今後の課題

- タイトコーナー2箇所の曲率緩和(オフェンス・ディフェンス共通で有用)は
  未着手。§47.5の実測グリップ天井(持続15.4 m/s²)を踏まえると、ay_max=12→
  14-15程度への引き上げ+`smoothing_distance`等のゼロ幾何レバー実験
  (from_claude氏提案)の方が、幾何編集より低リスクで効果が見込める可能性。
- 未来機械工房型「コリドー中心追従」(`line_follow_w`を0側へ)の蛇行抑制効果
  検証は未着手(dev3実験キュー)。
- タイトコーナーに近接する3区間(wp215-222・wp232-239・wp259-276)・S字帯の
  ディフェンスバイアスは、kaleidoscopeの`adjust_clearance`(占有格子ベースの
  自動探索、平滑性・曲率をコストに含む)を使った再挑戦の余地あり(未着手)。

### 47.9 ユーザー目視フィードバックによる仕上げ編集(2026-08-10続報)

kaleidoscope GUIでのユーザー目視確認により、以下の追加編集を実施(いずれも
`traj_mincurv_straightened.csv`ベース、offense/defense両方へ反映)。

**wp135-166の直線化**(ユーザー提案の最適点探索、from_claude式の推奨に対し
実測で「平坦点採用」の方が安全マージンが大きいと判定): 完全にkappa=0の直線に。

**wp349-18の3コブ統合**: wp346・wp3・wp16-17に分散していた3つの小コブを、
Laplacian平滑化(alpha=0.25、passes=10)で頂点wp4付近の1つのコーナーへ統合
(ユーザー提案「頂点wp6」に近い)。より強い平滑化を試したが局所的な凹凸
(extrema数)は減らず、逆に不安全化(壁近接)したため、この設定で確定。

**反復ローカル平滑化の限界に直面**: 接続部の歪み(wp25-27・wp40-42)を単純な
Laplacian平滑化(alpha 0.4-0.5、数pass)で繰り返し修正しようとしたが、
一箇所を直すと別箇所(wp4・wp172・wp344等)に問題が伝播し、最終的に不安全化
(is_safe=False)する事態を2回経験。都度、直前の安全な状態へ全パイプラインを
再実行して巻き戻した。

**解決策: Hermite補間ブレンド**: 単純平滑化の反復ではなく、位置・接線方向の
両方を連続にする3次Hermite補間(始点=元経路の接線、終点=直線区間の方向
ベクトル)へ切替。wp24-28・wp39-43に適用し、wp20-46全体で鋭いスパイクの
無い滑らかな遷移を実現(is_safe=True維持)。wp166→wp167-190コーナーの接続部
(wp161-173)にも同手法を試みたが、どの範囲を選んでも壁クリアランス不足
(is_safe=False)となり断念、こちらは軽微な残留歪み(kappa最大0.12 rad/m程度)
を残したまま確定。

**根本原因バグの発見: GENTLE_CORNERSの陳腐化**: オフェンス版(ベースのコピー)
はwp20-46で完全に滑らかだったが、ディフェンス版だけ同区間でkappa+0.17級の
大きな振れ(「右に振れてから左に戻る」とユーザーが正確に目視指摘)が発生。
原因は`build_defense_v2.py`の`GENTLE_CORNERS`に残っていた`(34,35,"R")`——
wp26-41の直線化で消滅したはずの小コーナーへ、存在しないコーナーとして生の
イン側バイアスが適用され続けていた。除外し、`protected_points()`へ
wp18-47も追加(全体Laplacian平滑化からも保護)して解消、offense/defense両方の
wp20-46 kappaが完全一致することを確認。

**教訓**: ベースジオメトリを編集した後は、`GENTLE_CORNERS`・`protected_points()`
の整合性確認と、offense/defense両ファイルの該当区間kappa突き合わせを必ず行う
(`scripts/traj_tools/README.md`に手順を追記)。回帰3256件PASS、ユーザー最終確認
「おｋです」で本ラウンドの経路仕上げ作業を完了。

## 48. ay_max引き上げ実験(タスク#308続報、2026-08-10)

### 48.1 動機・スイープ設計

ユーザー方針「コーナーも可能な限り25km/hに近づける」を受け、ay_max(現行7.0)を
実測グリップ天井(§47.5、持続15.4 m/s²)まで引き上げる実験。tri-param
(D1=15.0/D2=12.0/D3=9.0、ay_profileも同値で連動、他パラメータ固定)、n=1広域探索。
トラジェクトリは25km/hディフェンス版(§47.9更新済み)を使用。

途中、`install/`配下のシンボリックリンク未作成(colcon buildが個別ファイル単位で
symlinkを張るため、ビルド後に新規追加したtraj_offense_35kmh.csv/
traj_defense_25kmh.csvが未リンクのままだった)によりMPCコントローラが起動時に
即クラッシュする事故が発生(ユーザー指摘「はっしんしないですね」で発覚)。
手動でシンボリックリンクを追加し復旧。**教訓: 新規追加したデータファイルは
`colcon build`(または手動symlink作成)をしないとinstall/へ反映されない**
(CLAUDE.md §1.4の「Pythonソース変更は即反映」はこのケースには当てはまらない)。

### 48.2 結果: ハード制約はクリア、ソフト指標は明確な悪化トレンド

| 条件 | STUCK | COLLISION-SUSPECTED | 直線層wobble平均 | wp224/wp247到達速度 |
|---|---|---|---|---|
| D1(ay=15.0) | 0 | 4 | 33.49cm | 25.1/25.1 km/h |
| D2(ay=12.0) | 0 | 8 | 28.57cm | 24.9/24.9 km/h |
| D3(ay=9.0) | 0 | 8 | **16.13cm** | 22.6/25.3 km/h |

**wp224/wp247の到達速度はay_max=9.0の時点で既に22.6-25.3km/hに達しており、
12.0・15.0へ引き上げても+0.3-2km/h程度で頭打ち**(v_max=25.0自体が支配的な
上限になっているため)。一方、直線層wobbleはay_maxに比例して悪化
(§46.22で確認済みの傾向がさらに強く再現)。

### 48.3 周回平均速度(ユーザー方針転換: 「コーナー速度」から「周回平均速度」評価へ)

| 条件 | 全ラップ平均 | 速い方半分平均 | 最速ラップ |
|---|---|---|---|
| D1(ay=15.0) | 16.1 km/h(最悪) | 18.0 km/h | 21.3 km/h |
| D2(ay=12.0) | **19.4 km/h** | 20.8 km/h | 21.5 km/h |
| D3(ay=9.0) | 18.8 km/h | **21.4 km/h** | **21.8 km/h** |

**ay_max=15.0は最も攻めた設定にもかかわらず周回平均速度が最悪**——蛇行による
ロスがコーナー速度向上分を上回り逆効果になっていると推定。D2/D3はほぼ互角だが
D3の方が蛇行は半分程度軽いため、**D3(ay_max=9.0)を新ベースとして採用**
(ユーザー確定)。ay_max/ay_profileともに9.0へ更新(旧7.0から)。

### 48.4 30km/hコーナー目標の検討・保留

ユーザーより将来的に「コーナーも30km/hで走行する必要がある」との意向が示された。
タイトコーナー2箇所(wp224: R=2.57m, wp247: R=2.92m)で30km/hを出すには
それぞれay=27.0/23.8 m/s²が必要——実測グリップ天井(15.4 m/s²)を大きく超えており
ay_maxの引き上げでは物理的に到達不能。この2箇所を除く大半のコーナー(R=4-8m級)は
ay_max=15-20程度あれば30km/h射程内だが、今回は「タイトコーナー2箇所を除く
大半のコーナーで30km/hを目指す」現実的な目標に絞り、タイトコーナー2箇所の
トラジェクトリ再設計(TUM再実行等)は別タスクとして切り離すことで合意。

**副次的な戦略的示唆(ユーザー観察)**: 曲率が高くコース幅が実質的に狭い箇所
(まさにこの2箇所)は、攻撃側から見ても追い抜きが困難——ディフェンス戦略上、
この2箇所は追加のブロッキング処理をせずとも自然な防御ポイントとして機能する
可能性が高い。§47.7でこの2箇所をディフェンスバイアス対象から除外した判断とも
整合する。今後のディフェンス経路設計で「守るべき場所」の優先順位付けに活用できる
知見として記録。

### 48.5 次ステップ

ay_max=9.0を新ベースとして、直線層wobble(16.13cm)を出力調整(Q[e_y]→R[delta]→
r_delta_swing_boost→steer_low_pass_gain→r_drateの順、§37-46と同じ手順)で
削り込む再チューニングキャンペーンへ移行(本日中の予選投入が目標、ユーザー方針)。

## 49. Q[e_y]再チューニング(ay_max=9.0新ベース)+予選ログ分析でfootprint_risk
    自己ロックの実害を確認、selflock_release_enabled有効化(2026-08-10)

### 49.1 Q[e_y]ラウンドA(400k/600k/250k、n=1・87-90周と長時間)

tri-param(D1=600k/D2=400k[現行]/D3=250k)。10分想定が誤って2時間経過(87-90周)
してしまったが、結果的に長時間データとなった。

| Q[e_y] | 直線層wobble | \|ekf_ey\|平均(全区間) | COLLISION-SUSPECTED実イベント |
|---|---|---|---|
| 600k | 20.84cm(最悪) | 最大(全区間で最も経路から離れる) | **0件**/87周 |
| 400k(現行) | 17.66cm | 中間 | 8件/88周(0.091/周) |
| 250k | **12.05cm**(最良) | **最小**(全区間で最も正確に追従) | 12件/90周(0.133/周) |

**Q[e_y]を上げるほど蛇行・追従精度とも悪化**(理論上の「強く追いかける」意図に反し、
実際は自己励起振動が追従誤差そのものを増やす)。一方**COLLISION-SUSPECTEDはQ[e_y]を
下げるほど増加**という逆相関があり、ハード制約優先の原則(CLAUDE.md §2ルール4)から
250kを即採用とはしなかった。

### 49.2 Q[e_y]ラウンドB(100k/150k/200k)着手

蛇行がQ[e_y]に対し単調(頭打ちなし)であることを確認済みのため、250kよりさらに
下の帯(100k/150k/200k)を追加スイープ中(結果は次回反映)。

### 49.3 予選ログ0809-01/0809-02分析: footprint_risk自己ロックの実害を確認

CLAUDE.md §4.4 qualifying-log-analysisスキル手順で分析(config確認:
`traj_mincurv.csv`[本日のトラジェクトリ変更前]、v_max=25/ay_max=7.0、
§46確定Q/R)。ハード制約: STUCK 0件・COLLISION-SUSPECTED 7件(両ログとも同数)。
直線層wobble 8.03cm/8.40cm、ローカルdev3実測8.67cmとほぼ一致(過去は乖離が
大きかった実績があり、今回は良い一致)。
...
| | 試行数 | 成功率 | giveup理由 |
|---|---|---|---|
| 0809-01 | 24 | 16.7%(4件) | lat_ttc_C2×13, FOOTPRINT_RISK×4, room_exhausted×3 |
| 0809-02 | 15 | 46.7%(7件) | **FOOTPRINT_RISK×7(全件)** |

0809-02のgiveup全7件のうち6件がwp333に集中。`[LAT-TTC-ACT]`ログを追跡すると
`space=2.8-3.4m`(必要幅1.45mの約2倍)・`vopp≈0`(相手ほぼ停止)・
`curvature_blocked=False wall_blocked=False`・footprint_riskのみが理由、という
**§45で発見・修正済みの自己ロック条件と完全一致**。タイムスタンプを突き合わせると、
この6回連続giveup(1786280583.6〜698.5、約115秒間)はLap5(183.5秒、他ラップの
約3倍)の全期間にぴったり収まっており、**このラップの遅延を実際に引き起こしていた
ことを確認**。Phase3実地検証(dev3で2回試行・空振り)が探していた事象が、実は
既に予選本番で発生していたことになる。

### 49.4 selflock_release_enabled有効化

上記実害の確認を受け、ユーザー判断により`selflock_release_enabled`を
`false`→`true`へ先行して有効化(Phase3のdev3再現検証を経ずに実データの
実害確認を根拠とする判断)。`test_selflock_release_20260809.py`の
`test_config_gate_declared_default_false`を`test_config_gate_declared_enabled`
へ更新(デフォルトfalseの固定化テストだったため)。回帰3256件PASS。
dev3でのQ[e_y]ラウンドB完了後、この有効化の影響(OT成功率・giveup理由内訳の
変化)も合わせて検証する。

## 50. Q[e_y]再検証確定(200,000)+3軸評価フレームワーク導入+ay_maxスイープ(2026-08-10)

### 50.1 Q[e_y]ラウンドA/B/C/C2: 蛇行と経路乖離のトレードオフ発見

ay_max=9.0への引き上げ(§48)後の再検証として、Q[e_y]を段階的に絞り込んだ。

- **ラウンドA**(D1=600k/D2=400k/D3=250k、想定10分が実際は93分走行——確認遅延の
  反省点): 直線層蛇行は600k(20.84cm)→400k(17.66cm)→250k(12.05cm)と単調改善。
  COLLISION-SUSPECTEDは600k=0件・400k=8件・250k=12件と逆に悪化傾向。
- **ラウンドB**(D1=200k/D2=150k/D3=100k、14分): 蛇行はさらに単調改善(7.84→7.21→
  6.62cm)。COLLISION-SUSPECTEDは3条件とも0件だが露出時間がラウンドAの約1/6.7で
  確証薄い。
- **経路乖離量|ekf_ey|の発見**: ラウンドA/B全6点の|ekf_ey|平均を比較すると、
  600k(0.501m)→400k(0.425m)→250k(0.391m)→**200k(0.307m、最良)**→150k(0.326m)→
  100k(0.386m)と、200,000付近を谷とする**明確なU字型**であることが判明。蛇行(振れ幅)
  と経路乖離(定常追従精度)は別指標であり、Q[e_y]を下げすぎると蛇行は滑らかでも
  ラインから恒常的に離れる現象が別途進行していた([[mpc-q-r-weight-tuning-semantics]]
  の一般則と整合)。
- **ラウンドC**(D1=225k/D2=200k/D3=175k、30分想定→実際2.3-3.0分でタイマー異常
  発火、後述51節参照とは別件): 有効データ不足のため破棄・C2として再実行。
- **ラウンドC2**(同条件、実測10分確認): 蛇行は175k(7.53cm)<200k(7.97cm)<
  225k(8.12cm)、経路乖離は225k(0.293m)<200k(0.312m)<175k(0.324m)と、同一
  セッション内でも明確なトレードオフを再現。ハード制約は3条件とも0件。

**確定**: Q[e_y]=200,000(旧400,000から変更)。commit 9ea6ddf、回帰3256件PASS。

### 50.2 3軸評価フレームワークへの移行(ユーザー指示)

ラウンドC2の走行中、ユーザーが目視で「コーナー車速が18km/hくらいになってしまう」と
指摘。Q[e_y]は速度を決めるパラメータではないため無関係と判明したが、この気づきを
契機に、以後のパラメータ判断を**車速(コーナー到達速度)・経路乖離量・蛇行**の3軸で
行う方針へ変更。`qual_analysis/three_axis_check.py`を新設(直線/緩/中/急の曲率帯
別車速、|ekf_ey|全体統計、直線層蛇行を1コマンドで出力)。

### 50.3 ay_maxスイープ(9.0/11.5/14.0)3軸評価: 速度の伸びは小さく、9.0維持が妥当

3軸評価の初回適用として、ay_maxを14.0/11.5/9.0(基準)でスイープ(33分走行、
想定10分から大幅超過——タイマー通知に頼らず`docker ps`で手動確認したが確認自体が
遅れた。反省点)。

| ay_max | 車速(急curve) | 経路乖離平均 | 蛇行 | COLLISION-SUSPECTED |
|---|---|---|---|---|
| 14.0 | 22.2km/h | 0.529m | 18.24cm | 0件 |
| 11.5 | 21.4km/h | 0.426m | 8.77cm | 1件 |
| 9.0(基準) | 20.6km/h | 0.292m | 7.87cm | 0件 |

ay_maxを上げても速度の伸びは小さい(9.0→14.0で+1.6km/hのみ)一方、経路乖離は1.8倍・
蛇行は2.3倍に悪化する。**ay_max=9.0を維持**、引き上げは費用対効果が悪いと判断。

### 50.4 コーナー車速18km/hの正体: 既知の物理制約、Q/ay_maxチューニングでは解決しない

wp247-257(タイトコーナー)の曲率でay_max=9.0の場合、v=√(9.0/0.34)≈18.5km/hが
物理的な理論上限となり、実測18km/hとほぼ一致。§48で既に記録済みの「グリップ天井
(競合実測15.4 m/s²)によりタイトコーナー2箇所は25-30km/hに届かない」という制約の
再確認であり、バグではない。ここを上げるには経路そのものの半径拡幅が必要だが、
過去3回試行し壁衝突で失敗している(§47.4)。今回は着手せず記録のみに留める。

## 51. ディフェンストラジェクトリのバイアス境界に系統的な曲率欠陥を発見・修正(v2→v3)

### 51.1 発見の経緯

ユーザーの「トラジェクトリに改善は必要ですか」という問いを受け、`traj_defense_25kmh.csv`
(§47生成、v2)の曲率プロファイルを全区間スキャンしたところ、隣接点比で1.6倍以上の
局所スパイクが12箇所見つかった。原本(`traj_mincurv_straightened.csv`)と比較すると
うち8箇所(wp63/88/192/196/199/211/280/306)がv2のバイアス処理由来の新規欠陥、
残り4箇所(wp18/133/303/320)は原本に元々存在する自然な形状(欠陥ではない)と判明。

**最重要**: wp280は長年最重要ホットスポットとして扱われてきたwp269-282帯の中に
あり、原本では-0.02→-0.07→-0.21となめらかに強まる区間が、v2では-0.09→-0.16(5倍)
→+0.01(符号反転)→-0.10とジグザグになっていた。本日のQ[e_y]/ay_max実験
(§50)は全てこの欠陥入りのv2経路で実施されたため、wp269-282帯の挙動の一部は
この経路欠陥の影響を受けていた可能性がある。

### 51.2 原因分析

v2の設計(`build_defense_v2.py`)は「taperなしの生1.02mバイアス+全体への軽い
Laplacian平滑化(alpha0.15-0.25・6-10pass)で不連続を均す」方式だった。8箇所の
うち6箇所は隣接するGENTLE_CORNERS同士の隙間(2-4pt)、2箇所は単一コーナーの
境界(バイアス開始・終了点)で、いずれも軽い平滑化だけでは吸収しきれない段差が
系統的に残っていた。clearance検証(`is_safe`)は壁に当たらないことしか見ないため、
この種の曲率の歪みは見逃されていた。

### 51.3 修正方式(v3): smoothstepタパー

`build_defense_v3.py`を新設。各ゆるいコーナーの内部で、バイアス振幅を
smoothstep(3t²-2t³)で0→最大→0とテーパーする(taper幅はコーナー長の1/3、
最小4pt、2×taper幅がコーナー長を超える場合はバイアス自体を見送る)。
smoothstepは両端で導関数=0なので、コーナー境界で原ジオメトリと位置・接線方向が
なめらかに一致し、隣接コーナーとの隙間が数ptしかなくても不連続を作らない。
全体Laplacian平滑化は「仕上げ」として残すが、実際にはamp=0.85・平滑化なし
(alpha=0, passes=0)で最初に安全かつ新規スパイクゼロの条件が見つかった——
taperそのものが連続性を担保しており、v2で主役だった平滑化はむしろ不要と判明。

過去の教訓(タイトコーナー拡幅でのraised-cosine taper、3回不採用、`design_docs`
§47.4)との違い: あちらは「壁に極めて近いタイトコーナーの半径そのものを拡大する」
操作でtaper境界の曲率悪化がボトルネックになったが、今回は「ゆるいコーナー内側への
小さな平行移動」でtaper区間内の目標カーブが原カーブから大きく乖離しないため、
リスク構造が異なり実際に機能した。

### 51.4 (200,210,"L")コーナーの除外

taper幅を伸ばしても、このコーナーの境界(wp201/209)で原本ピーク曲率(0.146、
R≈6.8m)を超える曲率(0.20/0.18)が残った。原因は内側へのオフセットカーブが
曲率を増幅する幾何学的性質(半径R・オフセットdで新曲率=1/(R-d)、Rが小さいほど
影響が大きい)で、このコーナーが「ゆるい」というよりR≈6.8mとタイトな部類のため
影響が顕著だった。無理に均すより除外する方が安全と判断し、GENTLE_CORNERSから
削除(15→14区間)。

### 51.5 検証結果

- 元スパイク8箇所は全てkappa=0.02-0.05台(原本相当)まで復元、新規スパイク0件。
- 保護区間(wp18-48・wp133-168・タイトコーナー2箇所)は全て変位0.000000mを確認。
- wp269-282帯: 隣接する(259,276,"L")は既存のタイトコーナー近接除外ロジックで
  元々スキップ対象、(281,305,"R")はtaperにより281から滑らかにバイアスが立ち
  上がる形になり、v2のジグザグは完全に解消。
- kaleidoscope.trajectory_clearance検証: is_safe=True, min_clearance=0.53m
  (原本0.53mとほぼ同水準、安全マージンをほぼ犠牲にせず欠陥のみ解消)。
- pandas.read_csv直接確認: 350行・7列・NaN0件、実行時の消費経路と同一の
  読み込み方法で正常性を確認済み。

### 51.6 今後の課題

§50のQ[e_y]/ay_max実験は全てv2(欠陥あり)の経路で実施された。トレンド自体
(蛇行・経路乖離のU字型トレードオフ等)は経路欠陥と独立した制御パラメータの
性質である可能性が高く覆るとは考えにくいが、wp269-282帯固有の挙動・数値は
v3経路での再検証が望ましい。次回dev3走行時に確認する。

## 52. 経路修正後(v3)のQ[e_y]再検証: U字型トレードオフはほぼ解消、175,000へ更新

§51の経路修正(v3)を受け、Q[e_y]=175k/200k/225kを同じ3点で再検証した
(14分・15周、ハード制約は3条件ともCOLLISION-SUSPECTED 0件)。

| Q[e_y] | ラップタイム平均 | 車速(急curve) | 経路乖離平均 | 蛇行 |
|---|---|---|---|---|
| 225k | 53.61秒 | 21.9km/h | 0.259m(最良) | 8.85cm |
| 200k | 53.09秒 | 22.1km/h | 0.272m | 8.51cm |
| 175k | **52.53秒**(最良) | **22.2km/h**(最良) | 0.273m | **7.67cm**(最良) |

§50で見えていた「経路乖離が200k付近を谷とするU字型」は、v3では**ほぼ解消**した
(200k-225k間の差0.013mまで縮小、175k-200k間はわずか0.001m差)。v2の欠陥
(wp280含む)を低いQ[e_y]では追従しきれず、余計な乖離として観測されていた
副作用だったと考えられる。また、経路修正自体で3条件とも車速が底上げされた
(§50の同条件比、急curve帯で19.5-22.2km/h→21.9-22.2km/hへ改善)。

**175,000がラップタイム・車速・蛇行の3項目で最良**、経路乖離のみ225k比で
5%劣るが200kとはほぼ差なし。総合してQ[e_y]=175,000へ更新(旧200,000から)。
回帰スイート3256件PASS。

n=1(単一セッション)のため、CLAUDE.md §2の教訓通り確定的な結論ではないが、
「経路修正でトレードオフの形自体が変わりうる」という重要な事例として記録する。

## 53. wp170出口境界キンクの発見・修正(直線化の副作用、offense生成経路の是正含む)

### 53.1 発見の経緯

ユーザーが実走行で「WP170前後で一度震える」「WP262前後も確認して」と指摘。
現行アクティブ経路の曲率を確認したところ、wp262は原本と同様なめらか(0.043→
0.052→0.073→0.092→0.101と単調増加)で異常なしと判明。wp170は完全な原本
`traj_mincurv.csv`ではなめらか(-0.098→-0.122→-0.119→-0.135)なのに対し、
現行(wp135-166直線化後)ではwp169=-0.029(直線区間の名残)→wp170=-0.200
(1.7倍の急変)というスパイクが新たに生じていた。

全区間を「意図的に編集した区間(wp135-166コア・wp320-350+0-10・wp18-48)を
除外して原本と比較」する形でスキャンしたところ、この種の「未編集のはずが
曲率が変わっている」箇所はwp167-170のみで、他に同種の欠陥は無いことを確認。

### 53.2 原因

`straighten()`(直線化)は境界で位置は一致させるが接線方向を考慮しない単純chord
直線化のため、直線区間(タンジェント=直線方向)から原カーブ(タンジェント=カーブ
方向)への遷移がwp166/167の1点間で強制され、その帳尻合わせが数点先(wp170)の
曲率スパイクとして現れていた。直線化の入口側(wp135付近)は原本も既にほぼ直線
だったため対称的な問題は生じていない。

### 53.3 修正とその過程での学び

`fix_straighten_exit_kink.py`を新設し、wp20-46で実績のあるHermite補間ブレンド
(位置・接線方向とも連続)を適用。**当初wp158-178の広いウィンドウを試したところ、
なめらかにする代わりにコーナー内側へ膨らみすぎ、kaleidoscope検証でFOOTPRINT_COLLISION
(wp162-164)を引き起こした**——taper/ブレンド窓は広ければ良いわけではなく、実際の
壁クリアランスをkaleidoscopeで都度検証しながら最小限の幅に絞る必要があるという
教訓が改めて実証された(build_defense_v3のtaper設計でも同種の教訓があったが、
今回は逆方向[広すぎ]での再現)。複数の窓幅(158-178まで7パターン)を安全性・
スパイク解消の両方でスクリーニングし、**wp164-172が両方を満たす最小の窓**と判明
(is_safe=True・min_clearance=0.528m[原本と同水準]、wp170の近傍比1.01)。

### 53.4 副次的発見: offense生成経路の設定ミス

修正をoffense/defense両トラジェクトリへ反映する過程で、`build_two_trajectories.py`
の`TRAJ_PATH`が未編集の完全な原本`traj_mincurv.csv`を指したまま(2026-08-09時点
から更新漏れ)だったと判明。このスクリプトを実行すると当日の直線化・consolidate・
Hermiteブレンドが全て失われる状態だった。実際の`traj_offense_35kmh.csv`は手動
コピーで直線化後ベースを反映済みだったため実害は無かったが、以後
`traj_mincurv_straightened.csv`からの直接コピーで生成する運用に変更した。

### 53.5 検証結果

- pandas.read_csv直接確認: offense/defense両方とも350行/7列/NaN0件。
- kaleidoscope.trajectory_clearance: is_safe=True・min_clearance=0.53m(原本同水準)。
- 回帰スイート3256件PASS(制御コード本体は無変更)。

## 54. 経路修正後の全軸パラメータ再チューニングキャンペーン開始(タスク#316)

### 54.1 発端: dev3実地検証でディフェンス経路の直線層蛇行悪化を確認

wp170修正後、初めてディフェンス経路(`traj_defense_25kmh.csv`)をdev3で実地
検証(D1/D2=ディフェンス、D3=原本`traj_mincurv.csv`、いずれもv_max=25の同一
チューニング)。ハード制約は全クリアだったが、直線層蛇行がD1/D2(9.06-9.36cm)
>> D3(4.93cm)と約1.85倍悪化。ただしD3は本日の経路編集(直線化・統合・Hermite
ブレンド・wp170修正)を一切含まない完全な原本のため、「イン側バイアス単体の
コスト」と「本日の編集パッケージ全体のコスト」が交絡しており、単純比較では
原因を特定できないと判明(切り分けは保留)。

一方、wp269-282(最重要ホットスポット)・S字wp340-40はD1/D2の方がD3より
明確に良好(恐らく直線化・統合作業の効果)。

### 54.2 方針転換: 経路の切り分けより先に、経路修正後の全軸パラメータ再検証へ

ユーザー判断により、offense/defense切り分け実験は保留し、経路修正
(直線化・統合・イン側バイアス・ay_max/ay_profile 7.0→9.0)後に一度も
再検証されていない全MPCパラメータ(Q[e_y]/Q[e_psi]/Q[t]/Q[delta_actual]/
R[v]/R[delta]/r_drate/QN/steer_low_pass_gain/accel_low_pass_gain/
line_follow_w)を、過去実績の効果量順に1軸ずつ再チューニングする方針へ転換。
**今後の全ラウンドはD1/D2/D3ともディフェンス経路(`traj_defense_25kmh.csv`)を
共通ベースとする**(offense/原本との比較は行わない)。

過去実測の効果量ランキング(§38.8等)に基づき、以下の順序で合意:

**①steer_low_pass_gain → ②Q[e_y] → ③r_drate → ④r_delta_swing_boost →
⑤Q[e_psi] → ⑥line_follow_w → ⑦R[delta] → ⑧accel_low_pass_gain →
⑨Q[t] → (低優先)Q[delta_actual]/QN/R[v]**

### 54.3 dev3ラウンド報告の標準フォーマット追加(ユーザー指示)

以降の全dev3ラウンドで、ハード制約判定の直後に「ファステストラップ・平均
ラップ・経路乖離率(平均/p90)・平均車速・蛇行(直線層平均振れ)」の5項目表を
毎回報告する運用を追加(ユーザー明言、`.claude/memory/
feedback_dev3_round_summary_format.md`に保存済み)。

### 54.4 Round1: steer_low_pass_gain 0.6/0.8/1.0(n=2確定)

対象: `steer_low_pass_gain`。候補値0.6(現行)/0.8(過去検証済み近似)/
1.0(未来機械工房型のフィルタ完全無効化)。3ドメインともディフェンス経路+
ay_max/ay_profile=9.0固定。

**n=1(slpg_r1)**: ハード制約は全クリアだが、D3(1.0)にwp220-240(最大86.30cm)・
wp269-282(最大77.10cm、ekf_ey最大1.747m)・S字帯(最大80.30cm、ekf_ey最大
2.268m)の大外れ値が出現。過去実績(§38.8続報「低pass=0.8でwp180に最大129cm
の外れ値+STUCK急増(1→34)」)と同型のパターンに見え、当初は1.0を危険信号
として不採用方向に傾いた。

**n=2(slpg_r2)、CLAUDE.md §2 rule1の教訓通りの結果**: 同一3設定で再走行した
ところ、n=1で見えたD3の大外れ値は**一切再現しなかった**(全区間で最大でも
34cm程度)。むしろD3(1.0)が最重要ホットスポット(wp269-282: 12.57cm、D1=14.92/
D2=14.98より明確に良い)・直線層蛇行(7.20cm、最良)・ラップタイム(最速51.75s・
平均52.13s、いずれも最良)全てで最良か同等という結果に。n=1の外れ値はセッション
依存(対戦車との交錯)のノイズだったと判断。

**5項目サマリー(n=2)**:

| 項目 | D1(0.6) | D2(0.8) | D3(1.0) |
|---|---|---|---|
| ファステストラップ | 51.87s | 51.87s | **51.75s** |
| 平均ラップ | 55.14s | 53.42s | **52.13s** |
| 経路乖離率(平均/p90) | 0.287m/0.604m | 0.297m/0.618m | **0.286m/0.605m** |
| 平均車速(直線) | 23.9km/h | 24.0km/h | **24.0km/h** |
| 蛇行(直線層平均振れ) | 7.72cm | 7.24cm | **7.20cm** |

**判定**: `steer_low_pass_gain=1.0`を採用(0.6から更新)。未来機械工房型の
「フィルタ完全無効化」仮説(§47.5)が、経路修正後・ay_max=9.0のこの構成では
支持される結果となった。回帰3256件PASS。

### 54.5 次ステップ

Round2(Q[e_y])へ進む。現行175,000(§52確定、経路修正後・旧gain=0.6時代)を
`steer_low_pass_gain=1.0`基準下で再検証する。
