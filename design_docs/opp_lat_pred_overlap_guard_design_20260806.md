...
- **ログ収集・個別精査ともに完了**、wp269-284への集中(全体68%、真の
  ニアミス候補に絞ると70%)という定量的根拠がタスク#300・#306に加わった。
- CLAUDE.md §1.3の教訓(giveup/`cleared`判定周りへの安易なガード追加は
  過去に重大リグレッション[82-83節]を招いた実績あり)を踏まえ、**本節の
  時点では判定ロジックの変更は行わない**。次段階は「要注意」27件のうち
  上位数件(特にfootprint_taper発火の9件)についてofsset/corridor推移まで
  さらに深掘りし、具体的な対策案(giveupタイミングの条件見直し等)を
  設計してからdev3で慎重に検証する(タスク#300、継続)。
